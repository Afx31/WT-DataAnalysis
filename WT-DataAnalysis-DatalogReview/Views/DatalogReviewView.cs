...
public partial class DatalogReviewView : UserControl
{
    ...
    {
        SplitContainer spl_Main = new()
        {
            Name = "spl_Main_DatalogReview",
            Dock = DockStyle.Fill,
            SplitterDistance = 10
        };
    ...
    public void MapDataPointsToChart(int displayThisSpecificLap)
    {
        ...
        void DoDataPoints(List<ChartDataConfig> chartDataConfigs)
        {
            foreach (ChartDataConfig chartDataConfig in chartDataConfigs)
            {
                ...
                chart.ChartAreas["ChartArea" + counter.ToString()].AxisY.Interval = _csvData.GetDataPointsInterval(enumValue);
        ...
        if (chart.Series.Any(x => x.Name == "Series4:4:Gear"))
            chart.Series["Series:4:Gear"].Enabled = false;
        chart.ChartAreas["ChartArea4"].AxisY.Interval = 10;
